...
STAT_NAME_MAP = {
    ...
    "PHYSICAL_MITIGATION": "Physical Mitigation Scaling",
...
def process_stat(stats_dict, stat_xml):
    ...
    if rangedScaling is not None:
        statScaling = rangedScaling.split(":")[
            1
        ]  # Ignore the range... This might break things eventually but probably okay for now
...
def clean_items_df(df):
    df = (
        df.lazy()
        .filter(pl.col("category").is_in(["ARMOUR", "WEAPON", "ITEM"]))
        .filter(pl.col("slot").is_not_null())
        .filter(~pl.col("slot").is_in(["TOOL", "MAIN_HAND_AURA", "BRIDLE"]))
        .filter(pl.col("iLvl") > 499)
        .with_columns(
            pl.col("itemName").map_elements(
                lambda x: normalize("NFKD", x)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Parse the items database .xml file and save as csv"
    )
    parser.add_argument(
        "--items_fp",
        "-f",
        type=str,
        help="File path to xml of items db (download from LotroCompanion)",
        required=True,
    )
    parser.add_argument(
        "--save_fp",
